...
export default function KanbanWidget() {
  ...
  return (
    <div className="p-4 border rounded shadow bg-white dark:bg-slate-800 overflow-x-auto">
      <h2 className="widget-header font-semibold text-lg mb-2">Kanban Board</h2>
      <div className="flex space-x-4">
        {columns.map(column => (
          <div key={column.id} className="bg-slate-100 dark:bg-slate-700 p-2 rounded w-64 min-w-[16rem]">
            <h3 className="font-bold mb-2">{column.title}</h3>
            <ul className="space-y-1">
              {column.tasks.map((task: any) => (
                <li key={task.id} className="bg-white dark:bg-slate-900 p-1 rounded shadow text-sm">{task.title}</li>
              ))}
            </ul>
          </div>
        ))}
